...
def calculateLikelihood(wordList, vocabSize, prob_list):
    ...
    for word in wordList:
        P = float(wordList[word]+q)/float(n+q*vocabSize)
        ...
        for line in c:                  # for each word
            ...
            if line in training_words:
                if line in conservative_words:
                    ...
                    prob = float(q)/float(numCon+vocabSize)
                    prob = math.log(prob)
                    probC += prob
                ...
                if line in liberal_words:
                    ...
                    prob = float(q)/float(numLib+vocabSize)
                    prob = math.log(prob)
                    probL += prob
...
filename = sys.argv
trainingList, testList, q = filename[1], filename[2], filename[3]
q = float(q)
