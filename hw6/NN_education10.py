...
[1]                 [w1_1,w2_1,w3_1,w4_1,w5_1,wBias1_1]
[2]                 [w1_2,w2_2,w3_2,w4_2,w5_2,wBias1_2]              
[3]                 [w1_3,w2_3,w3_3,w4_3,w5_3,wBias1_3]           [w1_1,w2_1,w3_1,wB2_1] = Yes or No
[4]                                 [bias2 = 1.0]
[5]                 
[bias1 = 1.0]                      
                                  
You also have: 2 additional vectors for weighted sum only these being:
[3 elements long for input  -> hidden... with an additional 4th element for the bias of 1]
[1 element long for hidden -> output...]
'''
...
def weight_init(weights):
    ...
    if (len(weights) == 4):
        for i in range(0, len(weights), 1):
            weights[i] = np.random.normal(loc = 0, scale = 0.1, size = (6,))        # 6 weights (5 input nodes + 1 bias) from input to 3 hidden nodes excluding the bias
    if (len(weights) == 1):
        weights[0] = np.random.normal(loc = 0, scale = 0.1, size = (5,))            # 4 weights from hidden to output (3 hidden nodes + 1 bias)
    '''
    '''
    ...
    if (len(weights) == 10):
        weights[0] = np.array([ 0.07557171,  0.00313615,  0.05528243, -0.13766153,  0.05637146, 0.122669  ])
        weights[1] = np.array([-0.00876594, -0.00423154, -0.1383275 , -0.08876552,  0.0507692 , 0.08670285])
        weights[2] = np.array([ 0.03499204,  0.06432241, -0.1006322 , -0.07408504, -0.12924628, -0.20590552])
        weights[3] = np.array([ 0.02833682, -0.04335434,  0.00391716,  0.03485385, -0.04069407, 0.11423862])
        weights[4] = np.array([-0.04148819,  0.03700832, -0.02303058,  0.20598045,  0.082686  ,-0.06083899])
        weights[5] = np.array([ 0.02732419, -0.13201753,  0.14765454,  0.03325411,  0.18440086, -0.04180261])
        weights[6] = np.array([ 0.14255741,  0.13895387,  0.07235004,  0.19088431,  0.08223973, -0.04674287])
        weights[7] = np.array([ 0.09545663,  0.132499  , -0.01607091, -0.09848846,  0.13322108, 0.01101185])
        weights[8] = np.array([-0.07413881,  0.04638086,  0.11136378,  0.01220334, -0.05937796, -0.10544261])
        weights[9] = np.array([-0.1272994 ,  0.09553742, -0.1306474 , -0.02630282, -0.14433267, 0.01591896])
    if (len(weights) == 1):
        weights[0] = np.array([-0.07927331, -0.1277546 ,  0.13446392,  0.14134395, -0.01099068, 0.06493813,  0.27491588, -0.07470215, -0.05465567,  0.0182653 , -0.01389487])
    '''
    if (len(weights) == 4):
        weights[0] = np.array([-0.05112274,  0.06994763,  0.00683104, -0.10353957,  0.0868005 , -0.11019559])
        weights[1] = np.array([-0.06031955, -0.09835758, -0.10749608,  0.0523095 ,  0.12310858, -0.16336319])
        weights[2] = np.array([ 0.07544262, -0.1551947 , -0.06948627, -0.15425308, -0.02854544, -0.050749  ])
        weights[3] = np.array([-0.010197  , -0.00148523, -0.00290157, -0.16641048, -0.09494641, -0.06262128])
    if (len(weights) == 1):
        weights[0] = np.array([ 0.14094841,  0.11943845, -0.24600524,  0.0233491 , -0.09347039])
...
def weighedSum(inputs, weights):                                                                       # (1)
    ...
    # if middle layer:
    if (len(weights) == 4):
        sumVec = [0,0,0,0]              # Vector that holds all the weighted sums
    ...
    if (len(weights) == 1):
        ...
        for w_Vec in weights:
            ...
            sumVec[0] = sigmoid(w_Vec[0]*inputs[0] + w_Vec[1]*inputs[1] + w_Vec[2]*inputs[2] + w_Vec[3]*inputs[3] + w_Vec[4]*1.0)
    #print sumVec
    return sumVec
...
def back_prop(input_attributes, sig_hidden, sig_output, T, W_hidden, W_Output):
    ...
    errorVecHidden[-1] = 1+ sum_of_weighed_errors #////// you can't add diff_sigmoid over here because a bias doesn't come from a sigmoid... so what do you use instead
...
# Initialise Weights
weight_vector_to_hidden = [0,0,0,0]     # 0s converted later in weight_init()           # Array: weight matrix to hidden layer
...
while( iteration < 1800):
    ...
    if (iteration == 610):
        learning_rate = learning_rate/0.20
